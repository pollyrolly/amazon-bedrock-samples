...
The [Amazon Redshift Data API](https://docs.aws.amazon.com/redshift/latest/mgmt/data-api.html) is suitable for use cases where you don’t need a persistent connection to a cluster. 
In this project we use [AWS Step Functions](https://aws.amazon.com/step-functions/) together with Amazon Redshift Data API to orchestrate the different steps in the data collection for automated Cluster Review of [Amazon Redshift](https://aws.amazon.com/redshift/) data warehouse. These StepFunctions state machines are the part of the infrastructure as code (IaC) solution that a customer creates automatically in their AWS account to automate the Redshift Review process.

The data collection requires querying the Amazon Redshift cluster system tables and views. The results are stored the S3 bucket, and then integrated in a Dashboard. In total eighteen SQL are  executed.
